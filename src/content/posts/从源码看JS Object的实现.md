---
# 必填。项目名称。
title: "从Chrome源码看JS Object的实现"
# 可选，和文章一样使用。
slug: 从Chrome源码看JS Object的实现
...
order: 1
# 可选。卡片简介 + 详情页描述。
description: "从Chrome源码看JS Object的实现:V8的代码结构,JS Object类图,创建JS Object,字符串哈希cache查找,字符串哈希表查找,数字索引哈希查找,ES6 Map的实现,Object和ES6 Map的速度比较"
...
> C/C++/Java等语言是没有这种json的数据类型的，其它一些有的：如在Pthyon里面叫做字典，在Ruby/Perl里面叫散列表，当然这只是个名称而已，本质上可以当作json类型。而C是“万物之母”，C里面没有的，就得通过某种方式实现

> 并且JS里面的Object是如何查找属性的，这个问题有人说是通过遍历key的字符串查找的，也有人说是通过哈希查找的。究竟它是怎么存储和查找的，能不能把Object当作一个map来使用？
...
```js
../../v8/src/runtime/runtime-literals.cc 72 constant_properties: 0xdf9ed2aed19: [FixedArray]
– length: 6
...
- FixedArray主要用于表示数据的存储位置，在它上面还有一个Map，这个Map用于表示数据的结构。这里的Map并不是哈希的意思，更接近于地图的意义，用来操作FixedArray表示的这段内存。V8根据constant_properties的length，去开辟相应大小空间的Map：
  ...
  - 把这个申请后的Map打印出来：
    ...
    ```js
    ../../v8/src/heap/heap.cc 3472 map is 0x21528af9cb39: [Map]
    – type: JS_OBJECT_TYPE
    – instance size: 48
    ...
    - 从第4行可以看到，它的大小确实和我们算的一样。并且它还有一个叫做descriptors表示它的数据结构。descriptor记录了每个key-value对，以及它们在FixedArray里面的index. 后续对properties的操作基本上通过descriptor进行。
...
> 如下的判断：
  ...
  - 往这个哈希表插入一个元素前，会先检查表的容量是否足够，不够进行扩容，在EnsureCapacity的函数里面：
    ...
    Handle<Derived> new_table = HashTable::New(nof * 2, USE_DEFAULT_MINIMUM_CAPACITY);
...
> 假设data变成：
  ...
  - 把生成的data Object打印出来是这样的：
    ...
    ```js
    ../../v8/src/runtime/runtime-literals.cc 105 boilerplate obj: 0x3930221af3a9: [JS_OBJECT_TYPE]
        ...
        - 由于第一个属性是1: Monday，所以上面的index = 1，执行最后一行计算新容量的元素：
          ...
          - 这种用数组表示的属性在源码里面叫FastElements，而用哈希表的叫做SlowElements。什么时候会变成slowElements？当hole太多的时候，例如给data增加一个key为2000的属性：
            ...
            - data就会转成慢元素，因为3之后就是2000，hole太多就被转换了，如下面的ShouldConvertToSlowElements函数：
              ...
              ```js
              static bool ShouldConvertToSlowElements(JSObject* object, uint32_t capacity, uint32_t index, uint32_t* new_capacity) {
                if (index < capacity) {
                  return false;
                }
                ...
                *new_capacity = JSObject::NewElementsCapacity(index + 1);
                ...
                int dictionary_size = SeededNumberDictionary::ComputeCapacity(used_elements) * SeededNumberDictionary::kEntrySize;
...
> 这里有一个比较有趣的事情，就是V8的Map的核心逻辑是用JS实现的，用JS来实现JS，比写C++要高效多了，但是执行效率可能就没有直接写C++的高，可以来看一下set函数的实现：
  ...
  - 第三行添加一个log函数，确认确实是执行这里的代码。%开头的LOG，表示它是一个C++的函数，这个代码写在runtime.h和runtime.cc里面。这些JS代码最后会被组装成native code。在V8里，除了Map/Set之外，很多ES6新加的功能，都是用的JS实现的，如数组新加的很多函数。

#### 8. Object和ES6 Map的速度比较
...
- 再准备一个keys数组，用来存储查找的key，如下，分成三种情况分别测验：1. keys都能查到；2. 只有一半的keys能找到；3. 全部的keys都查找不到：
  ...
  - 然后进行查找，重复多次，并打印时间：
    ...
    - 最后的结果如下：
      ...
      ||100% keys found|50% keys found|0% keys found|
      |----|----|----|----|
      |map|517ms|375ms|317ms|
      |object|414ms|1.02s|1.49s|
...
> 综上，Object存储和查找一个属性时，属性不超过128个，使用Search Cache，当属性是较为连续的数字时，使用数组，此种方式最快。其它情况使用哈希表，并且数字和字符串的哈希不一样。

> 可以把Object当成哈希map使用，但是在效率上可能会比不上ES6的Map，并且还有Object原型查找的问题。

- 【转载】原文链接：[https://www.rrfed.com/2017/04/04/chrome-object/](https://www.rrfed.com/2017/04/04/chrome-object/)