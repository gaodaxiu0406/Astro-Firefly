...
- 面试题
  `forEach`，`for in`，`for`，`for of`的区别（为什么遍历数组的时候不采用`for in`）
  - 按理说 索引应该是数字number数据类型的，但是`for in`循环中key会变成`string`字符串数据类型
  ...
  - 如果给`arr`加上个属性 `arr.b='100'`  属于数组的私有属性`for in`可以打印出来
...
- `forEach`不支持`return`
- `for in`循环遍历出来的包含私有属性
- `for of` 既能`return`（并且是`val`（值）`of`数组），又能遍历私有属性（不能遍历对象）
...
- 如果想删除数组中的某一项，可以用`filter`
- 如果想更新或者修改某个数组，可以用`map`
...
总结:求和、求叠加、两个数组合成一个数组，可以用`reduce`

> 扩展

- 数组扁平化:将一个二维数组变为一维数组怎么写？可以使用`reduce` -> 将`[[1,2,3],[4,5,6],[7,8,9]]变为[1,2,3,4,5,6,7,8,9]`
